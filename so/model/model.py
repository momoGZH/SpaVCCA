...
from .layer import Encoder, Decoder, ZScoreDeZScore
from .loss import *
...
class Model(nn.Module):
    ...
    def __init__(self, input_dim, h_dim, multiple=8, num_heads=2, n_domain=1, mu_t=None, std_t=None):
        """
        :param input_dim:
        :param h_dim:
        :param multiple:
        ...
        """
        super().__init__()
        self.z_layer = ZScoreDeZScore(mu_t, std_t)
        self.encoder = Encoder(input_dim, h_dim, multiple=multiple, n_domain=n_domain, num_heads=num_heads)
    ...
    def encode_batch(
        ...
        if out == 'latent':
            ...
            for i, (input_nodes, output_nodes, block) in enumerate(tqdm(dataloader, desc="Infer latent", total=len(dataloader))):
                block = block.to(device)
                x, y, idx = block.ndata['feature'], block.ndata['batch'], block.ndata['index']
                x = self.z_layer.zscore(x, y)
    ...
    def fit(
        ...
        :param beta:
        :param random_seed:
        :param reconstruct_loss: orig or zscore
        :param alpha_cca
        :param graph_loss
        :return:
        """
        ...
        if reconstruct_loss == 'orig':
            print('reconstruct_loss == orig')
        elif reconstruct_loss == 'zscore':
            print('reconstruct_loss == zscore')
        elif reconstruct_loss == 'mix':
            print('reconstruct_loss == mix')
        else:
            raise NotImplementedError
        optim = torch.optim.Adam([
            {'params': self.encoder.parameters(), 'lr': lr, 'weight_decay': 5e-4},
            {'params': self.decoder.parameters(), 'lr': lr, 'weight_decay': 5e-4},
        ])
        scheduler = get_linear_schedule_with_warmup(optim, num_warmup_steps=int(n_epoch * 0.1),
                                                    num_training_steps=n_epoch)
        epoch_loss_l = []
        PAR_loss = FastGroupedBlockContrastiveLoss()
            ...
            for epoch in tq:
                if mode == 'mini_batch':
                    ...
                    for i, (input_nodes, output_nodes, block) in tk0:
                        block = block.to(device)
                        x_orig, y = block.ndata['feature'], block.ndata['batch']
                        x = self.z_layer.zscore(x_orig, y)
                        z, mean, var = self.encoder(x, block)
                        recon_x, recon_x_scale = self.decoder(z, y, block)
                        recon_x_orig = self.z_layer.dezscore(recon_x, y)
                        # loss
                        spot_quality_mask = block.ndata['spot_quality']
                        if reconstruct_loss == 'orig':
                            recon_loss_mse = F.mse_loss(recon_x_orig[spot_quality_mask], x_orig[spot_quality_mask]) * x_orig.size(-1) / 1
                        elif reconstruct_loss == 'zscore':
                            recon_loss_mse = F.mse_loss(recon_x[spot_quality_mask], x[spot_quality_mask]) * x.size(-1) / 1
                        # elif reconstruct_loss == 'mix':
                        #     recon_loss_mse = F.mse_loss(recon_x[spot_quality_mask], x_orig[spot_quality_mask]) * x.size(-1) / 1
                        kl_loss = kl_div(mean, var)
                        ref_batch = int(random.choice(y.unique().tolist()))   # TODO int(random.choice(y.tolist()))
                        cca_loss = cca_loss_to_reference(z, y, reference_batch=None) * alpha_cca
                        if graph_loss:
                            graph_loss_ce = graph_rl(block, mean, device)
                        else:
                            graph_loss_ce = torch.tensor(0.0, device=device, requires_grad=True)
                        # loss_cov = cov_offdiag_loss(mean)  # 1)
                        # loss_var = var_floor_loss(mean, gamma=1.0)  # 2)
                        # loss_bt = barlow_twins_loss_single_view(mean, lambd_offdiag=0.005)
                        sub_par_loss = PAR_loss(block, mean, y)
                        loss = {
                            'kl_loss': beta * kl_loss * alpha_kl,
                            'recon_loss': alpha * recon_loss_mse * alpha_re,
                            'cca_loss': cca_loss * alpha_cca,
                            'sub_par_loss': sub_par_loss * alpha_spl,
                        }
                    ...
                    temp = pd.DataFrame.from_dict(epoch_loss, orient='index').T
                    temp.index = [epoch]
                    epoch_loss_l.append(temp)
                else:
                    epoch_loss = defaultdict(float)
                    x_orig, y = dgl_graph.ndata['feature'], dgl_graph.ndata['batch']
                    x = self.z_layer.zscore(x_orig, y)
                    ...
                    # loss
                    kl_loss = kl_div(mean, var)
                    ref_batch = int(random.choice(y.unique().tolist()))
                    cca_loss = cca_loss_to_reference(z, y, reference_batch=None) * alpha_cca
                    if graph_loss:
                        graph_loss_ce = graph_rl(dgl_graph, mean, device)
                    else:
                        graph_loss_ce = torch.tensor(0.0, device=device, requires_grad=True)
                    # loss_cov = cov_offdiag_loss(mean)  # 1)
                    # loss_var = var_floor_loss(mean, gamma=1.0)  # 2)
                    ...
                    temp.index = [epoch]
                    epoch_loss_l.append(temp)
        epoch_loss_df = pd.concat(epoch_loss_l)
        adata.uns['epoch_loss_df'] = epoch_loss_df
        return adata
