...
from .model_utils import generate_dataloader
...
def SpaVCCA(
    ...
    # model config
    model = Model(x_dim, h_dim, multiple=multiple, num_heads=num_heads, n_domain=n_domain, mu_t=adata.uns['mu_bg'], std_t=adata.uns['std_bg'])
    ...
    adata.obsm['X_SpaVCCA'] = model.encode_batch(test_loader, out='latent', device=device, evaluate=evaluate,
                                              size=len(adata))
